...
inline void processRectifier(float* const* channelData, int numChannels, int numSamples, std::vector<float>& state,
                             float drive, float blend, float inScale, float outScale, RectFn rectF,
                             RectF1Fn rectF1) noexcept
{
    ...
    const auto shaper = [dryD, blendD, rectF](double v) noexcept { return dryD * v + blendD * rectF(v); };
    ...
    for (int ch = 0; ch < channels; ++ch)
    {
        ...
        {
            const float driven = juce::jlimit(-kDrivenClamp, kDrivenClamp, dsputil::sanitise(data[n]) * drive);
                          ...
                          const StyleParams& params) noexcept
{
    ...
    processRectifier(
        channelData, numChannels, numSamples, adaaState, drive, blend, kSmudgeInScale, kSmudgeOutScale,
                           ...
                           const StyleParams& params) noexcept
{
    ...
    processRectifier(
        channelData, numChannels, numSamples, adaaState, drive, blend, kRectifyInScale, kRectifyOutScale,
        [](double v) noexcept { return adaa::rectifyF(v); }, [](double v) noexcept { return adaa::rectifyF1(v); });
}
} // namespace ember
