...
void XLfo::setDefaultShape()
{
    ...
    const std::array<LfoPoint, 2> defaultShape{{{0.0f, 1.0f, ModCurve::SCurve}, {0.5f, -1.0f, ModCurve::SCurve}}};
...
float XLfo::evaluate(const Shape& shape, float phase01) const noexcept
{
    ...
    else
    {
        ...
        segEnd = (i + 1 < n) ? shape.points[static_cast<size_t>(j)].position : shape.points[0].position + 1.0f;
    }
...
void XyController::setParameters(const XyControllerParams& newParams) noexcept
{
    ...
    if (static_cast<int>(params.axis) < 0 || static_cast<int>(params.axis) >= static_cast<int>(XyAxis::Count))
...
void MidiSource::setParameters(const MidiSourceParams& newParams) noexcept
{
    ...
    if (static_cast<int>(params.kind) < 0 || static_cast<int>(params.kind) >= static_cast<int>(MidiSourceKind::Count))
...
float MidiSource::tick(int numSamples, const MidiState& state) noexcept
{
    ...
    return juce::jlimit(0.0f, 1.0f, smoother.process(juce::jlimit(0.0f, 1.0f, raw), params.smoothingMs, numSamples));
}
