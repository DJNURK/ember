...
constexpr std::uint32_t writeSlot   (std::uint32_t s) noexcept { return  s        & 3u; }
constexpr std::uint32_t pendingSlot (std::uint32_t s) noexcept { return (s >> 2)  & 3u; }
constexpr std::uint32_t readSlot    (std::uint32_t s) noexcept { return (s >> 4)  & 3u; }

constexpr std::uint32_t packSlots (std::uint32_t write, std::uint32_t pending,
                                   std::uint32_t read, bool fresh) noexcept
{
...
// ---------------------------------------------------------------- lifetime
void ModulationEngine::prepare (double sampleRate, [[maybe_unused]] int maxBlockSize,
                                int numModulatableParams)
{
    ...
    for (auto& lfo : lfos)              lfo.prepare (sr);
    for (auto& eg : envGenerators)      eg.prepare (sr);
    for (auto& ef : envFollowers)       ef.prepare (sr);
    xyPad.prepare (sr);
    for (auto& ms : midiSources)        ms.prepare (sr);
    for (auto& macro : macroSources)    macro.prepare (sr);
...
void ModulationEngine::reset() noexcept
{
    ...
    for (auto& eg : envGenerators)      eg.reset();
    for (auto& ef : envFollowers)       ef.reset();
    xyPad.reset();
    for (auto& ms : midiSources)        ms.reset();
    for (auto& macro : macroSources)    macro.reset();
...
void ModulationEngine::updateControlBlock (const float* perBandRms, int numBands, int numSamples) noexcept
{
    ...
    if ((expected & kFreshBit) != 0u)
    {
        ...
        while (! planSlots.compare_exchange_weak (expected, desired,
    ...
    for (int i = 0; i < kNumModSources; ++i)
    {
               ...
               && plan.connections[static_cast<size_t> (connectionIndex)].sourceIndex == flat)
        {
            ...
            const float contribution = conn.enabled ? applyModCurve (value, conn.curve) * conn.amount
                                                    : 0.0f;
...
float ModulationEngine::tickSource (const EvaluationPlan& plan, int flatIndex, int numSamples,
...
float ModulationEngine::fieldOffset (const EvaluationPlan& plan, int flatIndex,
                                     ModSourceField field) const noexcept
{
    ...
    const int targetIndex = plan.fieldTarget[static_cast<size_t> (flatIndex)]
...
void ModulationEngine::rebuildPlan()
{
    ...
    for (int slot = 0; slot < numConnections; ++slot)
    {
        ...
        while (k > 0
               && position[static_cast<size_t> (plan.connections[static_cast<size_t> (k - 1)].sourceIndex)]
...
void ModulationEngine::setConnection (int slot, const ModConnection& c)
{
    ...
    for (int i = 0; i < numConnections; ++i)
    {
        ...
        if (existing.sourceIndex == replacement.sourceIndex
            && existing.targetIndex == replacement.targetIndex
...
void ModulationEngine::fromValueTree (const juce::ValueTree& tree)
{
    ...
    if (node.isValid())
    {
        ...
        for (int i = 0; i < numChildren && numConnections < kMaxModConnections; ++i)
        {
            ...
            c.amount      = static_cast<float> (static_cast<double> (
                                child.getProperty (ModStateIds::amount, 0.0)));
            c.curve       = static_cast<ModCurve> (static_cast<int> (
                                child.getProperty (ModStateIds::curve, 0)));
            c.smoothingMs = static_cast<float> (static_cast<double> (
                                child.getProperty (ModStateIds::smoothing,
                                                   static_cast<double> (defaults.smoothingMs))));
