...
Preset makePreset (const char* fileName, const char* name, const char* category,
                   std::vector<float> crossovers, std::vector<BandSpec> bands)
{
    Preset p;
    p.fileName = fileName;
    p.name = name;
...
void setEnvGenerator (Preset& p, int i, int triggerIndex, float thresholdDb,
                      float attackMs, float decayMs, float sustainPercent, float releaseMs)
{
...
void setXlfoFree (Preset& p, int i, float rateHz, int steps, float depthPercent,
                  float phaseDeg, float smoothPercent)
{
...
void setXlfoSynced (Preset& p, int i, float cycleBeats, int steps, float depthPercent,
                    float phaseDeg, float smoothPercent)
{
...
void setMacro (Preset& p, int i, float percent) { p.extras.push_back ({ pid::macro (i), percent }); }
...
std::vector<Preset> buildPresetBank()
{
    ...
    // =================================================================== Drums
    {
        auto p = makePreset ("Drums_01_Kick_Weight_and_Click.xml", "Kick Weight and Click", "Drums",
                             { 95.0f, 2200.0f },
    ...
    {
        auto p = makePreset ("Drums_02_Snare_Crack.xml", "Snare Crack", "Drums",
                             { 180.0f, 3500.0f },
    ...
    {
        auto p = makePreset ("Drums_03_Room_Mic_Glue.xml", "Room Mic Glue", "Drums",
                             { 350.0f },
    ...
    {
        auto p = makePreset ("Drums_04_Overhead_Sheen.xml", "Overhead Sheen", "Drums",
                             { 1800.0f },
    ...
    {
        auto p = makePreset ("Drums_05_Parallel_Smash_Bus.xml", "Parallel Smash Bus", "Drums",
                             { 220.0f },
    ...
    {
        auto p = makePreset ("Drums_06_Hat_and_Shaker_Polish.xml", "Hat and Shaker Polish", "Drums",
                             { 4000.0f },
    ...
    // ==================================================================== Bass
    {
        auto p = makePreset ("Bass_08_Sub_Anchor.xml", "Sub Anchor", "Bass",
                             { 70.0f, 700.0f },
    ...
    {
        auto p = makePreset ("Bass_09_Bass_Grind_Upper.xml", "Bass Grind Upper", "Bass",
                             { 90.0f, 900.0f },
    ...
    {
        auto p = makePreset ("Bass_10_Round_and_Warm.xml", "Round and Warm", "Bass",
                             { 500.0f },
    ...
    {
        auto p = makePreset ("Bass_11_Synth_Bass_Iron.xml", "Synth Bass Iron", "Bass",
                             { 120.0f },
    ...
    {
        auto p = makePreset ("Bass_12_Long_Sub_Bloom.xml", "Long Sub Bloom", "Bass",
                             { 110.0f },
    ...
    {
        auto p = makePreset ("Bass_13_Bass_Reamp_Bite.xml", "Bass Reamp Bite", "Bass",
                             { 100.0f, 1200.0f },
    ...
    // ================================================================== Vocals
    {
        auto p = makePreset ("Vocals_14_Vocal_Presence_Lift.xml", "Vocal Presence Lift", "Vocals",
                             { 200.0f, 2500.0f },
    ...
    {
        auto p = makePreset ("Vocals_15_Intimate_Whisper.xml", "Intimate Whisper", "Vocals",
                             { 400.0f },
    ...
    {
        auto p = makePreset ("Vocals_16_Rap_Vocal_Edge.xml", "Rap Vocal Edge", "Vocals",
                             { 180.0f, 3000.0f },
    ...
    {
        auto p = makePreset ("Vocals_17_Gritty_Lead_Vocal.xml", "Gritty Lead Vocal", "Vocals",
                             { 300.0f },
    ...
    {
        auto p = makePreset ("Vocals_18_Backing_Vocal_Bed.xml", "Backing Vocal Bed", "Vocals",
                             { 600.0f },
    ...
    // ================================================================== Guitar
    {
        auto p = makePreset ("Guitar_19_Clean_Electric_Sparkle.xml", "Clean Electric Sparkle", "Guitar",
                             { 900.0f },
    ...
    {
        auto p = makePreset ("Guitar_20_Crunch_Rhythm_Stack.xml", "Crunch Rhythm Stack", "Guitar",
                             { 140.0f, 1800.0f },
    ...
    {
        auto p = makePreset ("Guitar_21_Singing_Lead_Sustain.xml", "Singing Lead Sustain", "Guitar",
                             { 500.0f },
    ...
    {
        auto p = makePreset ("Guitar_22_Acoustic_Body_and_Air.xml", "Acoustic Body and Air", "Guitar",
                             { 250.0f, 4000.0f },
    ...
    {
        auto p = makePreset ("Guitar_23_Amp_Room_Dirt.xml", "Amp Room Dirt", "Guitar",
                             { 700.0f },
    ...
    {
        auto p = makePreset ("MixBus_25_Tape_Cohesion.xml", "Tape Cohesion", "Mix Bus",
                             { 120.0f, 5000.0f },
    ...
    {
        auto p = makePreset ("MixBus_26_Console_Weight.xml", "Console Weight", "Mix Bus",
                             { 180.0f },
    ...
    {
        auto p = makePreset ("MixBus_27_Wide_and_Quiet.xml", "Wide and Quiet", "Mix Bus",
                             { 300.0f },
    ...
    {
        auto p = makePreset ("MixBus_28_Master_Polish.xml", "Master Polish", "Mix Bus",
                             { 90.0f, 700.0f, 6000.0f },
    ...
    // ================================================================ Creative
    {
        auto p = makePreset ("Creative_29_Broken_Radio.xml", "Broken Radio", "Creative",
                             { 400.0f, 3000.0f },
    ...
    {
        auto p = makePreset ("Creative_30_Ring_of_Foldback.xml", "Ring of Foldback", "Creative",
                             { 600.0f },
    ...
    {
        auto p = makePreset ("Creative_31_Pulse_Gate_Grind.xml", "Pulse Gate Grind", "Creative",
                             { 250.0f },
    ...
    {
        auto p = makePreset ("Creative_32_Resonant_Howl.xml", "Resonant Howl", "Creative",
                             { 500.0f },
    ...
    {
        auto p = makePreset ("Creative_33_Vowel_Sweep.xml", "Vowel Sweep", "Creative",
                             { 250.0f, 700.0f, 1800.0f, 4000.0f },
    ...
    {
        auto p = makePreset ("Creative_34_Bit_Rot_Texture.xml", "Bit Rot Texture", "Creative",
                             { 300.0f, 2500.0f },
    ...
    {
        auto p = makePreset ("Creative_35_Shimmer_Haze.xml", "Shimmer Haze", "Creative",
                             { 200.0f, 2000.0f },
...
void validateCrossovers (const Preset& preset, const std::array<float, kMaxCrossovers>& edges)
{
    for (int i = 1; i < kMaxCrossovers; ++i)
    {
        ...
        if (! (upper > lower))
        {
            fail (preset.name + ": crossover edges are not ascending ("
                  + juce::String (lower, 2) + " Hz then " + juce::String (upper, 2) + " Hz)");
            continue;
        }

        if (upper / lower < kMinEdgeRatio - 1.0e-4f)
            fail (preset.name + ": crossover edges " + juce::String (lower, 2) + " Hz and "
                  + juce::String (upper, 2) + " Hz are closer than a third of an octave");
    }
}
               ...
               const juce::String& id, float value)
{
    ...
    {
        fail (presetName + ": " + id + " = " + juce::String (value, 3) + " is outside ["
              + juce::String (range.start, 3) + ", " + juce::String (range.end, 3) + "]");
        return;
    }
...
int styleIndexForName (const juce::String& presetName, const juce::String& styleName)
{
    ...
    if (index < 0)
        fail (presetName + ": unknown style '" + styleName + "' (choices: "
              + choices.joinIntoString (", ") + ")");
...
juce::String renderPreset (const Preset& preset, const std::map<std::string, int>& targetIndices)
{
    ...
    if (static_cast<int> (preset.crossovers.size()) != preset.numBands() - 1)
        fail (name + ": " + juce::String (preset.numBands()) + " bands need "
              + juce::String (preset.numBands() - 1) + " crossovers but the table gives "
              + juce::String (static_cast<int> (preset.crossovers.size())));
    ...
    for (int band = 0; band < preset.numBands(); ++band)
    {
        ...
        set (pid::feedbackFreq (band),
             b.feedbackFreq == kUseDefault ? defaultValueOf (apvts, pid::feedbackFreq (band))
                                           : b.feedbackFreq);
...
int main (int argc, char** argv)
{
    ...
    std::printf ("make_presets: wrote %d presets in %d categories to %s\n",
                 static_cast<int> (documents.size()), categories.size(),
                 outputDir.getFullPathName().toRawUTF8());
